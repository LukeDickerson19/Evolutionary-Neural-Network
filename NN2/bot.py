...
class Bot(ParentSprite):
    ...
    def __init__(self, model, dna=None):
        ...
        self.radius = BOT_MAX_RADIUS
        super(Bot, self).__init__(model, dna)
        self.radius = BOT_START_RADIUS
        if dna is not None:
        	self.color = dna['color']
        	self.nn = NN(parents_NN=dna['parent_nn'])
        else:
    ...
    def update_eye_data(self, eye):
        ...
        x, y, r, theta = self.x, self.y, self.radius, self.angle
        
        # eye variable setup
        eye_sep = EYE_SEPARATION
        vis_dist = 12 * self.radius
    ...
    def update_visual_field(self, circles, eye, controller):
        ...
        max_visable_distance = 12 * self.radius
        ...
        for c in circles:
            ...
            if d - cr <= max_visable_distance:
                ...
                # if the circle c is just barely on the outside of the max_visable_distance
    ...
    def update_smell(self, model):
        ...
        for bot in model.bots:
            if bot != self:
               bot_smell += \
                (-2 / (1 + np.exp(-(2 * self.get_dist(bot)) / (3*self.radius))) + 2)
        self.bot_smell = 2 / (1 + np.exp(-4*bot_smell)) - 1 # sqash it

        food_smell = 0.0
        for food in model.foods:
            food_smell += \
            (-2 / (1 + np.exp(-(2 * self.get_dist(food)) / (3*self.radius))) + 2)
    ...
    def update_transform(self, model):
        ...
        if lwr == 0 or rwr == 0:
            ...
            # for b in model.bots:
            #     if b != self:
            #         dist = np.sqrt((new_x - b.x)**2 + (new_y - b.y)**2)
            ...
            #     new_x, new_y = 2*self.x - new_x, 2*self.y - new_y
    ...
    # update how much noise this bot is makeing from its
    ...
    def reproduce_asexually(self, model):
        ...
        for child in children:
			...
			for j in range(3):
				...
				if change_in_rgb < -50: change_in_rgb = -50
				new_c = self.color[j] + change_in_rgb
			...
			# brain
			child['parent_nn'] = self.nn

			model.bots.append(Bot(model, child))
